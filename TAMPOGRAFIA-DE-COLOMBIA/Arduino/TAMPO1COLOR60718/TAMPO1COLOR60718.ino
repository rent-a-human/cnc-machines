...
void IMPRIMIR() {
  ...
  if (bt2p1 == 1) {
    ...
    VerificarPedal();
    if ((INICIO_BARRIDO - (POSICION_CUCHILLA_ABAJO - POSICION_CUCHILLA_ARRIBA) > 0)) {
      sincrono = (INICIO_BARRIDO - (POSICION_CUCHILLA_ABAJO - POSICION_CUCHILLA_ARRIBA)) / APPCARRO;
      RetroCarroACC(sincrono);
      VerificarPedal();
      frecuencia = APPCUCHILLA / APPCARRO;
      ASCuchillaCarro(posicion_cuchilla_abajo - posicion_cuchilla_arriba);
    } else if (inicio_barrido > 0) {
      frecuencia = (inicio_barrido / (posicion_cuchilla_abajo - posicion_cuchilla_arriba)) * APPCARRO / APPCUCHILLA;
      ASCuchillaCarro(posicion_cuchilla_abajo - posicion_cuchilla_arriba);
    } else {
      CerrarCuchilla((posicion_cuchilla_abajo - posicion_cuchilla_arriba));
    }
