...
void Configurar() {
  ...
  switch (va0) {
    ...
    case 2://Pag 2
      ...
      Serial.print("Pagina 2 va1: ");
      Serial.println(va1p2);
      if (va1p2 == 0) {
        myNextion.setComponentValue("h0", BSP0); delay(2);
        myNextion.setComponentValue("n0", BSP0); delay(2);
        myNextion.setComponentValue("h1", BSP1); delay(2);
        myNextion.setComponentValue("n1", BSP1); delay(2);
        myNextion.setComponentValue("n2", BSP1 - BSP0); delay(2);
...
void RetrocederBandeja(double steps) {
  pausa = 8600 / velocidad;
  for (int i = 0; i < steps / 10; i++) {
    RetroBandeja(1);
  }
  pausa = 6200 / velocidad;
  for (int i = 0; i < steps / 10; i++) {
    RetroBandeja(1);
  }
  pausa = 4800 / velocidad;
  for (int i = 0; i < (steps / 10); i++) {
    RetroBandeja(1);
  }
  pausa = 3900 / velocidad;
  for (int i = 0; i < (steps / 10); i++) {
    RetroBandeja(1);
  }
  pausa = 3000 / velocidad;
  for (int i = 0; i < (steps * 2 / 10); i++) {
    RetroBandeja(1);
  }
  pausa = 3900 / velocidad;
