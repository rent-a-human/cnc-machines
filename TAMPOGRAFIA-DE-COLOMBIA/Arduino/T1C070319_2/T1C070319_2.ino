...
#define SENSOR_CARRO A1
#define SENSOR_CUCHILLA A2
#define SENSOR_TAMPO A3
#define PEDAL A0
...
void setup() {
  Serial.begin(19200);;
  Serial.println("Inicio de programa T1C070319_2");
...
void IMPRIMIR() {
  ...
  if (bt2p1 == 1) {
   ...
   if(modo==0){
      ...
      AvanceCarroACC(MAX);//////////////////PUNTO LEJANO
    }
    ...
    if (modo == 1) { //Modo plano
      if (tampo == 1) {
        RetroTampoACC(presionimprimir);
      }
    } else if (modo == 0) { //Modo Cilindrico
    ...
    VerificarPedal();
      RetroCarroACC(MAX);//ORIGEN
      //ReiniciarCarro();
    VerificarPedal();
    pausa = p3;
    pausacuchilla = 26;
    while (PACUCHILLA > posicion_cuchilla_arriba && PACARRO > 0) {
      AbrirCuchilla(1);
      RetroCarro(1);
    }
    pausacuchilla = 40;
